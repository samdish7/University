...
public class MakerlabFragment extends Fragment {
        ...
        if (isConnected) {
            ...
            contact.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (email.resolveActivity(Objects.requireNonNull(getActivity()).getPackageManager()) != null) {
    ...
    private void handleEmptyJSON() {
        json_availability[0] = !today_json_string.isEmpty();

        json_availability[1] = !other_hours_string.isEmpty();

        json_availability[2] = !printer_json_string.isEmpty();
        ...
        for (int count = 0; count < other_dates.length; count++) {
            if (!other_dates[count].isEmpty() && count != 0) {
                ...
                if (methods[count].equalsIgnoreCase("closed")) {
                    tv_status.setText(getText(R.string.closed));
                } else if (methods[count].equalsIgnoreCase("byapp")){
                    tv_status.setText(getText(R.string.schedule));
                } else {
    ...
    public void addDevicesLayout() {
        ...
        for (int count = 0; count < printer_names.length; count++) {
            if (!printer_names[count].isEmpty()){
                ...
                String text = "(" + printer_type[count] + ")";
                type.setText(text);
                ...
                if (!printer_avail[count].isEmpty()) {
                    text = "Available: " + printer_avail[count];
                    avail.setText(text);
                } else {
                    avail.setText(R.string.walkin);
                }
